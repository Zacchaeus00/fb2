...
class Model12(torch.nn.Module):
    def __init__(self, ckpt, num_train_steps, lr, lr_head=None, reduction='mean', warmup_ratio=0, rnn_type='none', rnn_bi=True, rnn_nl=1, rnn_dropout=0):
        ...
        if rnn_type == 'gru':
            self.rnn = torch.nn.GRU(self.config.hidden_dim, self.config.hidden_dim, rnn_nl, dropout=rnn_dropout,
                                    bidirectional=rnn_bi, batch_first=True)
        elif rnn_type == 'lstm':
            self.rnn = torch.nn.LSTM(self.config.hidden_dim, self.config.hidden_dim, rnn_nl, dropout=rnn_dropout,
                                    bidirectional=rnn_bi, batch_first=True)
        else:
            raise NotImplementedError
        self.dropout1 = StableDropout(0.1)
