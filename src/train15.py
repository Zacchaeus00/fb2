...
from data_utils import FB2Dataset, prepare_samples_with_prompt, tokenize_samples
...
prompt = pd.read_csv('../data/fb2_train_topics.csv')
prompt['desc_processed'] = prompt['desc'].apply(str.lower)
prompt = prompt.set_index('id')[['desc_processed']].squeeze()
raw_samples = prepare_samples_with_prompt(essay, train, prompt, tokenizer.sep_token)
samples = tokenize_samples(raw_samples, tokenizer, pooling='cls')
